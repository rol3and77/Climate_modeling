...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with main_col:
        ...
        render_infobox(
            "연구 목적",
            "본 프로젝트는 관측 데이터와 단순화된 물리 기반 에너지 균형 모델을 결합하여, 전지구 평균 기온의 역사적 변화를 재현하고 주요 기후 강제력과 물리 파라미터 변화가 장기 온난화 경로에 미치는 영향을 분석하는 것을 목적으로 한다. "
            "특히 시나리오 기반 예측과 파라미터 민감도 분석을 통해 기후 시스템의 핵심 동작 원리를 정량적으로 해석하는 데 초점을 둔다.",
        )
        render_infobox(
            "모델 구조와 물리적 가정",
            "본 모델은 육지, 해양 혼합층, 심해의 세 층으로 구성된 단순 에너지 균형 모델이다. 각 층은 서로 다른 열용량을 가지며, 이를 통해 기후 시스템의 시간 지연 효과와 열 저장 특성을 표현한다. "
            "모델에는 CO₂ 복사 강제력, 에어로졸 냉각 효과, 비CO₂ 인위적 강제력, 화산 강제력, 그리고 ENSO 유사 내부 변동성이 포함된다. "
            "이들 요소는 단순화된 함수 형태로 구현되어 있으며, 전지구 평균 규모에서 장기 기온 변화의 주요 경향을 설명하는 데 목적을 둔다."
        )
