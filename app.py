...
.module-card-link {
    ...
    .nav-title-clean {
        ...
        font-weight: 850;
        color: #0f172a;
        margin: 0.15rem 0 0.9rem 0.05rem;
    ...
    .exp-page-icon {
        ...
        height: 38px;
        border-radius: 10px;
        background: linear-gradient(180deg, #dfe9ff 0%, #d7e3fb 100%);
        color: #2f64d6;
        display: flex;
        align-items: center;
    ...
    .exp-top-card, .exp-panel-card {
        background: linear-gradient(180deg, #ffffff 0%, #f8fbff 100%);
        border: 1px solid #dbe5f0;
        border-radius: 16px;
        box-shadow: 0 12px 28px rgba(15, 23, 42, 0.05);
    }
    .exp-top-card { padding: 1rem 1rem 0.9rem 1rem; margin-bottom: 0.9rem; }
    .exp-top-title {
        font-size: 1.05rem;
        font-weight: 850;
        color: #1d4ed8;
    ...
    .exp-panel-card { padding: 0; overflow: hidden; }
    .exp-panel-header {
        display: flex;
        align-items: center;
        justify-content: space-between;
    ...
    .exp-kpi-delta.negative { color: #2563eb; }
    @media (max-width: 1200px) {
        .exp-condition-row { grid-template-columns: repeat(3, minmax(0, 1fr)); }
        .exp-split-grid { grid-template-columns: 1fr; }
    }
...
# --- Sidebar navigation ---
st.sidebar.title("탐색")
if st.sidebar.button("시작 페이지", use_container_width=True, key="sb_home"):
    set_page("시작 페이지")
for idx, nav_page in enumerate(NAV_PAGES, start=1):
    if st.sidebar.button(nav_page, use_container_width=True, key=f"sb_{idx}"):
        set_page(nav_page)
...
def set_page(target_page):
    ...
    st.query_params["module"] = page_to_module_slug.get(target_page, "home")
    st.rerun()

def render_left_panel():
    st.markdown('<div class="nav-title-clean">탐색</div>', unsafe_allow_html=True)

    if st.button("시작 페이지", use_container_width=True, key="left_nav_home"):
        set_page("시작 페이지")

    for idx, nav_page in enumerate(NAV_PAGES, start=1):
        if st.button(nav_page, use_container_width=True, key=f"left_nav_{idx}"):
            set_page(nav_page)

    st.markdown(
        """
        <div class="note-card">
            원하는 분석 모듈을 선택하면 이 왼쪽 패널에서 언제든 다른 페이지로 이동할 수 있습니다.
        </div>
        """,
        unsafe_allow_html=True
    )
...
def render_page_settings(current_page):
    controls = {}

    if current_page == "시나리오 기반 기후 변화 예측":
        st.markdown('<div class="settings-shell"><div class="settings-shell-title">설정</div>', unsafe_allow_html=True)
        scenario_options = [
            "탄소중립 안정화 시나리오",
            "저온난화 경로 시나리오",
            "현재 정책 유지 시나리오",
        ...
        controls["policy"] = st.select_slider(
            ...
            value=st.session_state.get("main_policy", "현재 정책 유지 시나리오"),
            key="main_policy"
        )
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="settings-shell"><div class="settings-shell-title">설정</div>', unsafe_allow_html=True)
        if st.button("실험 설정 초기화", use_container_width=True, key="main_reset_experiment"):
            st.session_state["main_exp_co2"] = 550
            st.session_state["main_exp_lambda"] = 1.5
            st.session_state["main_exp_aer"] = 1.0
            st.session_state["main_exp_klo"] = 2.0
            st.session_state["main_exp_enso"] = 0.12
        controls["exp_co2"] = st.slider("2100년 CO2 농도 (ppm)", 250, 1500, int(st.session_state.get("main_exp_co2", 550)), step=10, key="main_exp_co2")
        controls["exp_lambda"] = st.slider("기후 피드백 파라미터", 0.5, 3.0, float(st.session_state.get("main_exp_lambda", 1.5)), step=0.1, key="main_exp_lambda")
        controls["exp_aer"] = st.slider("에어로졸 강도", 0.0, 3.0, float(st.session_state.get("main_exp_aer", 1.0)), step=0.1, key="main_exp_aer")
        controls["exp_klo"] = st.slider("해양 열흡수 계수", 0.5, 4.0, float(st.session_state.get("main_exp_klo", 2.0)), step=0.1, key="main_exp_klo")
        controls["exp_enso"] = st.slider("ENSO 진폭", 0.0, 0.5, float(st.session_state.get("main_exp_enso", 0.12)), step=0.01, key="main_exp_enso")
        st.markdown('</div>', unsafe_allow_html=True)

    elif current_page == "모델 적합도 및 관측자료 비교":
        st.markdown('<div class="settings-shell"><div class="settings-shell-title">설정</div>', unsafe_allow_html=True)
        obs_list = list(obs_datasets.keys())
        default_obs = st.session_state.get("main_obs_choice", obs_list[0])
        controls["obs_choice"] = st.selectbox("관측 데이터셋 선택", obs_list, index=obs_list.index(default_obs), key="main_obs_choice")
        ...
        st.markdown('<div class="settings-shell"><div class="settings-shell-title">설정</div>', unsafe_allow_html=True)
        obs_list = list(obs_datasets.keys())
        default_diag = st.session_state.get("main_diag_obs_choice", obs_list[0])
        controls["diag_obs_choice"] = st.selectbox("검증용 데이터셋 선택", obs_list, index=obs_list.index(default_diag), key="main_diag_obs_choice")
...
if page == "시작 페이지":
    ...
    controls = render_page_settings(page)
    ...
    # parameter state
    if "main_exp_co2" not in st.session_state:
        st.session_state["main_exp_co2"] = 800
    if "main_exp_lambda" not in st.session_state:
        st.session_state["main_exp_lambda"] = 1.2
    if "main_exp_aer" not in st.session_state:
        st.session_state["main_exp_aer"] = -1.2
    if "main_exp_klo" not in st.session_state:
        st.session_state["main_exp_klo"] = 0.8
    if "main_exp_enso" not in st.session_state:
        st.session_state["main_exp_enso"] = 1.0
    ...
    if "exp_run_ready" not in st.session_state:
        st.session_state["exp_run_ready"] = True

    exp_co2 = int(st.session_state["main_exp_co2"])
    exp_lambda = float(st.session_state["main_exp_lambda"])
    exp_aer = float(st.session_state["main_exp_aer"])
    exp_klo = float(st.session_state["main_exp_klo"])
    exp_enso = float(st.session_state["main_exp_enso"])

    custom_params = [exp_lambda, exp_aer, exp_klo, exp_enso]
    baseline_params = [baseline_lambda, baseline_aer, baseline_klo, baseline_enso]

    res_exp, tl_exp, tm_exp, td_exp, _ = run_model(custom_params, -0.22, end_year=2100, end_co2=exp_co2)
    res_base, tl_base, tm_base, td_base, _ = run_model(baseline_params, -0.22, end_year=2100, end_co2=baseline_co2)
    ...
    # derived metrics
    temp_2100 = float(res_exp[-1])
    temp_2050 = float(res_exp[2050 - 1925])
    base_2100 = float(res_base[-1])
    base_2050 = float(res_base[2050 - 1925])
    ecs_proxy = float(3.7 / max(exp_lambda, 0.2))
    ecs_base = float(3.7 / max(baseline_lambda, 0.2))
    forcing_2100 = float(co2_forcing(exp_co2, temp_2100) + aerosol_effect(2025, exp_aer))
    forcing_base = float(co2_forcing(baseline_co2, base_2100) + aerosol_effect(2025, baseline_aer))
    imbalance_2100 = forcing_2100 - exp_lambda * temp_2100
    imbalance_base = forcing_base - baseline_lambda * base_2100

    def delta_html(v):
        cls = "negative" if v < 0 else ""
        sign = "+" if v >= 0 else ""
        return f'<div class="exp-kpi-delta {cls}">{sign}{v:.2f}</div>'

    st.markdown(
        """
        <div class="exp-page-header">
            <div class="exp-page-title-wrap">
                <div class="exp-page-icon">∿</div>
                <div>
                    <div class="exp-page-title">기후 시스템 파라미터 실험</div>
                    <p class="exp-page-subtitle">물리 파라미터를 조정하여 기후 시스템의 민감도와 미래 변화를 분석합니다.</p>
                </div>
            </div>
            <div class="exp-guide-pill">ⓘ 사용 가이드</div>
        </div>
        """,
        unsafe_allow_html=True
    )

    status_ready = "준비 완료" if st.session_state.get("exp_run_ready", True) else "대기 중"
    status_text = "모든 파라미터가 설정되었습니다." if st.session_state.get("exp_run_ready", True) else "실험 실행 전입니다."

    st.markdown(
        f"""
        <div class="exp-top-card">
            <div class="exp-top-title">현재 실험 조건</div>
            <div class="exp-condition-row">
                <div class="exp-cond-item">
                    <div class="exp-cond-label">복사 강제력 (CO₂)</div>
                    <div class="exp-cond-value">{exp_co2} ppm</div>
                    <div class="exp-cond-note">기준값: {baseline_co2} ppm</div>
                </div>
                <div class="exp-cond-item">
                    <div class="exp-cond-label">에어로졸 강제력</div>
                    <div class="exp-cond-value">{exp_aer:.2f}</div>
                    <div class="exp-cond-note">기준값: {baseline_aer:.2f}</div>
                </div>
                <div class="exp-cond-item">
                    <div class="exp-cond-label">피드백 매개변수 (λ)</div>
                    <div class="exp-cond-value">{exp_lambda:.2f}</div>
                    <div class="exp-cond-note">기준값: {baseline_lambda:.2f}</div>
                </div>
                <div class="exp-cond-item">
                    <div class="exp-cond-label">해양 열흡수 계수 (kₒ)</div>
                    <div class="exp-cond-value">{exp_klo:.2f}</div>
                    <div class="exp-cond-note">기준값: {baseline_klo:.2f}</div>
                </div>
                <div class="exp-cond-item">
                    <div class="exp-cond-label">ENSO 진폭</div>
                    <div class="exp-cond-value">{exp_enso:.2f}</div>
                    <div class="exp-cond-note">기준값: {baseline_enso:.2f}</div>
                </div>
                <div class="exp-status-card">
                    <div class="exp-status-title">● 실험 상태</div>
                    <div class="exp-status-text"><b>{status_ready}</b><br>{status_text}</div>
                </div>
            </div>
        </div>
        """,
    ...
    st.markdown('<div class="exp-split-grid">', unsafe_allow_html=True)

    # left settings panel
    st.markdown(
        """
        <div class="exp-panel-card">
            <div class="exp-panel-header">
                <div class="exp-panel-title-wrap">
                    <div class="exp-panel-step">1</div>
                    <div class="exp-panel-title">파라미터 설정</div>
                </div>
            </div>
    ...
    if st.button("초기화", key="exp_reset_button", use_container_width=True):
        st.session_state["main_exp_co2"] = baseline_co2
        st.session_state["main_exp_lambda"] = baseline_lambda
        st.session_state["main_exp_aer"] = baseline_aer
        st.session_state["main_exp_klo"] = baseline_klo
        st.session_state["main_exp_enso"] = baseline_enso
        st.rerun()

    st.markdown('<div class="exp-subgroup"><div class="exp-subgroup-title">☀️ 복사 강제력 관련</div>', unsafe_allow_html=True)
    st.session_state["main_exp_co2"] = st.slider("CO₂ 농도 (ppm)", 200, 1600, exp_co2, step=10, key="exp_slider_co2")
    st.session_state["main_exp_aer"] = st.slider("에어로졸 강제력", -3.0, 0.0, exp_aer, step=0.05, key="exp_slider_aer")
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="exp-subgroup"><div class="exp-subgroup-title">↻ 피드백 관련</div>', unsafe_allow_html=True)
    st.session_state["main_exp_lambda"] = st.slider("피드백 매개변수 λ", 0.2, 2.5, exp_lambda, step=0.05, key="exp_slider_lambda")
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="exp-subgroup"><div class="exp-subgroup-title">≈ 해양 관련</div>', unsafe_allow_html=True)
    st.session_state["main_exp_klo"] = st.slider("해양 열흡수 계수 kₒ", 0.1, 2.0, exp_klo, step=0.05, key="exp_slider_klo")
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="exp-subgroup"><div class="exp-subgroup-title">∿ 내부 변동성 관련</div>', unsafe_allow_html=True)
    st.session_state["main_exp_enso"] = st.slider("ENSO 진폭", 0.0, 2.0, exp_enso, step=0.05, key="exp_slider_enso")
    st.markdown('</div>', unsafe_allow_html=True)

    if st.button("▶ 실험 실행", key="exp_run_button", use_container_width=True):
        st.session_state["exp_run_ready"] = True
        st.rerun()

    st.caption("파라미터를 변경한 후 실험 버튼을 눌러 결과를 업데이트하세요.")
    ...
    st.markdown(
        ...
        <div class="exp-panel-card">
            <div class="exp-panel-header">
                <div class="exp-panel-title-wrap">
                    <div class="exp-panel-step">2</div>
                    <div class="exp-panel-title">결과 비교</div>
                </div>
            </div>
            <div class="exp-panel-body">
        """,
    ...
    top_controls_col1, top_controls_col2 = st.columns([1.25, 0.75])
    with top_controls_col1:
        chart_mode = st.selectbox(
            "비교 보기",
            ["전 지구 평균 기온 변화", "층별 온도 변화"],
            key="exp_chart_mode"
        )
    with top_controls_col2:
        exp_export_df = pd.DataFrame({
            "Year": years_exp,
            "Baseline_Global": res_base,
            "Experiment_Global": res_exp,
            "Experiment_Land": tl_exp,
            "Experiment_Ocean_Mixed": tm_exp,
            "Experiment_Deep_Ocean": td_exp,
        })
        st.download_button(
            "결과 내보내기",
            exp_export_df.to_csv(index=False).encode("utf-8"),
            file_name="experiment_results.csv",
            mime="text/csv",
            use_container_width=True
        )

    fig_exp, ax_exp = plt.subplots(figsize=(10.5, 4.8))
    if chart_mode == "전 지구 평균 기온 변화":
        ax_exp.plot(years_axis, obs_preview, color="#111827", lw=1.9, label="Historical Observation")
        ax_exp.plot(years_exp, res_base, color="#94a3b8", lw=2.0, ls="--", label="Baseline Scenario")
        ax_exp.plot(years_exp, res_exp, color="#2563eb", lw=2.3, label="Experiment Scenario")
        band = np.linspace(0.05, 0.75, len(years_exp))
        ax_exp.fill_between(years_exp, res_exp - band * 0.25, res_exp + band * 0.85, color="#93c5fd", alpha=0.25, linewidth=0)
        ax_exp.axvline(2000, color="#cbd5e1", ls="--", lw=1.2)
        ax_exp.set_title("Global Mean Temperature Change (1925-2100)", fontsize=14, fontweight="bold")
        ax_exp.set_ylabel("°C")
    else:
        ax_exp.plot(years_exp, tl_exp, color="#2563eb", lw=2.2, label="Land Surface")
        ax_exp.plot(years_exp, tm_exp, color="#94a3b8", lw=2.0, label="Ocean Mixed Layer")
        ax_exp.plot(years_exp, td_exp, color="#111827", lw=1.8, label="Deep Ocean")
        ax_exp.set_title("Layer-Specific Temperature Response (1925-2100)", fontsize=14, fontweight="bold")
    ...
    c2100 = temp_2100 - base_2100
    c2050 = temp_2050 - base_2050
    cecs = ecs_proxy - ecs_base
    cimb = imbalance_2100 - imbalance_base

    st.markdown(
        f"""
        <div class="exp-mini-kpis">
            <div class="exp-kpi-card">
                <div class="exp-kpi-label">2100년 기온 변화</div>
                <div class="exp-kpi-value">{temp_2100:.2f}°C</div>
                {delta_html(c2100)}
            </div>
            <div class="exp-kpi-card">
                <div class="exp-kpi-label">2050년 기온 변화</div>
                <div class="exp-kpi-value">{temp_2050:.2f}°C</div>
                {delta_html(c2050)}
            </div>
            <div class="exp-kpi-card">
                <div class="exp-kpi-label">평형 기후 민감도 (ECS)</div>
                <div class="exp-kpi-value">{ecs_proxy:.2f}°C</div>
                {delta_html(cecs)}
            </div>
            <div class="exp-kpi-card">
                <div class="exp-kpi-label">복사 불균형 (2100)</div>
                <div class="exp-kpi-value">{imbalance_2100:.2f}</div>
                {delta_html(cimb)}
            </div>
        </div>
        """,
    ...
    st.markdown(
        ...
        <div class="exp-panel-card">
            <div class="exp-panel-header">
                <div class="exp-panel-title-wrap">
                    <div class="exp-panel-step">3</div>
                    <div class="exp-panel-title">파라미터 영향도 분석</div>
                </div>
            </div>
            <div class="exp-panel-body">
        """,
    ...
    st.caption("각 파라미터를 단독으로 변화시켰을 때 2100년 기온 변화에 미치는 상대적 영향을 비교합니다.")

    sens_labels = ["CO₂ 농도", "피드백 매개변수 λ", "해양 열흡수 계수 kₒ", "에어로졸 강제력", "ENSO 진폭"]
    sens_variants = [
        run_model([baseline_lambda, baseline_aer, baseline_klo, baseline_enso], -0.22, end_year=2100, end_co2=800)[0][-1] - base_2100,
        run_model([1.2, baseline_aer, baseline_klo, baseline_enso], -0.22, end_year=2100, end_co2=baseline_co2)[0][-1] - base_2100,
        run_model([baseline_lambda, baseline_aer, 0.8, baseline_enso], -0.22, end_year=2100, end_co2=baseline_co2)[0][-1] - base_2100,
        run_model([baseline_lambda, -1.2, baseline_klo, baseline_enso], -0.22, end_year=2100, end_co2=baseline_co2)[0][-1] - base_2100,
        run_model([baseline_lambda, baseline_aer, baseline_klo, 1.0], -0.22, end_year=2100, end_co2=baseline_co2)[0][-1] - base_2100,
    ]

    fig_sens, ax_sens = plt.subplots(figsize=(10.5, 4.2))
    colors = ["#ef4444" if v > 0 else "#2563eb" for v in sens_variants]
    ax_sens.barh(sens_labels, sens_variants, color=colors, alpha=0.9)
    ax_sens.axvline(0, color="#94a3b8", lw=1)
    ax_sens.set_title("Parameter Impact on 2100 Warming", fontsize=13, fontweight="bold")
    ax_sens.set_xlabel("Change in 2100 Temperature (C)")
    ax_sens.grid(True, axis="x", alpha=0.18)
    st.pyplot(fig_sens)
    st.markdown('</div></div>', unsafe_allow_html=True)
