...
"""
        ...
        render_infobox(
            "해석",
            "배출량이 증가할수록 복사 강제력이 증가하여 온난화 경향이 가속되는 것을 확인할 수 있다. "
            "단, 본 결과는 단순화된 전지구 평균 모델에 기반한 것으로, "
            "절대적 예측값보다는 시나리오 간 상대 비교에 초점을 두어 해석해야 한다.",
        )
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with main_col:
        ...
        render_infobox(
            ...
            "장기 온난화 결과에 어떤 영향을 주는지 탐색하기 위해 구성되어있다. "
            "해양 열흡수, ENSO, 화산 강제력, 비CO₂ 인위적 강제력은 현재 화면에서는 기준값으로 고정되어 있다.",
        )
